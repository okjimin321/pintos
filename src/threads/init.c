...
main (void)
{
  ...
  /* Clear BSS. */  
  bss_init (); // uninitialized variable들을 0으로 초기화

  /* Break command line into arguments and parse options. */
  argv = read_command_line ();// 커널 커맨드를 인자 단위로 자름
  argv = parse_options (argv);// 옵션들(ex: -s -q)을 처리하고 남은 실행 인자만을 return

  /* Initialize ourselves as a thread so we can use locks,
     then enable console locking. */
  thread_init (); // 스스로를 thread화(main thread를 만든 다음에 ready list에 넣음)
  console_init ();// 입출력 장치 초기화

  /* Greet user. */
  printf ("Pintos booting with %'"PRIu32" kB RAM...\n",
          init_ram_pages * PGSIZE / 1024); // init_ram_pages는 초기화된 frame 수, PGSIZE는 frame 사이즈

  /* Initialize memory system. */
  palloc_init (user_page_limit);
  malloc_init ();
  paging_init (); // page table set up해줌
  ...
  intr_init (); // 인터럽트 핸들링을 위한 IDT를 준비
  timer_init ();
  kbd_init ();
  input_init ();
#ifdef USERPROG
  ...
  thread_start (); // idle 스레드 생성 및 인터럽트 on
  serial_init_queue (); 
  timer_calibrate ();// 타이머를 교정
  ...
  /* Finish up. */
  shutdown ();
  thread_exit ();// 현재 스레드 종료하고 다른 스레드로 schedule
}

...
{
  static char *argv[LOADER_ARGS_LEN / 2 + 1]; // 인자 배열
  char *p, *end;
  int argc; // 인자의 수수
  int i;
