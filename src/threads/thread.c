...
void
thread_start (void)// idle thread를 만들고 인터럽트를 켬
{
               ...
               thread_func *function, void *aux) 
{
  ...
  struct switch_threads_frame *sf;// fake stack frame
  tid_t tid;

  ASSERT (function != NULL); // 실제 스레드를 만드는 커널 스레드의 내부 함수
...
thread_block (void) 
{
  ASSERT (!intr_context ());  // 인터럽트 핸들러 안에 있는지 확인
  ASSERT (intr_get_level () == INTR_OFF);
...
void
thread_yield (void)// 스케줄러에게 cpu를 넘김
{
  struct thread *cur = thread_current ();
  enum intr_level old_level;
...
void
thread_foreach (thread_action_func *func, void *aux)// 전달 받은 함수를 모든 thread에서 수행
{
  struct list_elem *e;
...
thread_schedule_tail (struct thread *prev)
{
  ...
  /* Mark us as running. */
  cur->status = THREAD_RUNNING;// new thread를 running으로 바꿔줌

  /* Start new time slice. */
  thread_ticks = 0;// time slice 초기화
  ...
  if (prev != NULL && prev->status == THREAD_DYING && prev != initial_thread) 
    ...
    }// 스스로 지울 수 없으니, 죽은 다음 schedule될 때 free해줌
}
...
schedule (void)// block, exit, yield에서 호출(interrupt를 끈 상태에서 호출함)
{
  ...
  if (cur != next)
    prev = switch_threads (cur, next);// switch 발생
